...
|______|______|______|______|______|______|______|______|______|______|______|______|______|______|______|______|______|______|______|______|______|______|______|
**UPDATES**
Anti money hack | weapon hack | AB Hack (called every 3 seconds)
/mute bug fixed
VIP Timer
GRank added to /info
/rules bug fixed
bad words (/allowfw /addbw /removebw Forrbidden_Words table)
map icon + exit CP added to VIP Lounge
freezing bug fixed when buying something
**TO DO LIST**
/mygang Online/Offline tags
achievement system
rob-able drug house
/goto pveh /get pveh /check (playerid)
/housemenu (title, money stocking)
/shopmenu (getrevenu, see revenue, upgrades (+revenue rate))
/park bug
IRC system
/updates(with log)
message of the day
limit vehicles/businesses/houses
/serverpanel (to change everything from the game)
money bag
anti hack (health TP)
		...
		if(PlayerInfo[i][LoggedIn] == true){
			...
			if(!IsPlayerInAnyVehicle(i) && GetPlayerWeapon(i) != 46){
				...
				if(pPosX <= -0.800000  || pPosY <= -0.800000 || pPosZ <= -0.800000 && anim == 1008){
					format(acmsg, sizeof(acmsg), "[WARNING]: {FFFFFF}%s is potentially Airbreaking",GetName(i));
					SendAdminMessage(acmsg);
				}
			}
		}
			...
			if(IsPlayerConnected(otherID))
			{
				if(PlayerInfo[otherID][LoggedIn])
				{
					...
					else if(!strcmp(selection, "money", true))
					{
						...
						//GivePlayerMoneyEx(playerid, -mgot+amount);
					...
					else if(!strcmp(selection, "highpoint", true))
					{
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	//Dealership dialogs
		...
		switch(listitem){
	        case 0: return ShowPlayerDialog(playerid, DIALOG_GENERALCMDS, DIALOG_STYLE_MSGBOX, "General Commands", "/pm || /blockpm || /ask || /changemypass || /cmds - /commands || /kill || /stats || /rules || /scmds || /info || /help || /fishstats\n/hit || /acceptsex || /ransom || /helpers || /admins || /shopcmds || /report || /eventpos || /anims || /lotto || /mylnumber || /gangs", "Got it!", "");
	        case 1:{
	            if(gTeam[playerid] == LSPD){
